defmodule Typelixir.Processor do
  ...
  @doc """
  Code.string_to_quoted 함수로 주어진 경로의 파일을 파일로 읽어들여 ast로 만들고
  Macro.prewalk로 이 ast를 따라가며 타입 체킹한다.any()

  ast의 각 노드 종류에 따라 매칭되는 함수 process를 만들어 이 함수가 호출되는 구조로 작성되었다.
  따라서, 이 process 함수에 매칭되는 노드 종류와 그렇지않은 노드 종류를 조사하고 이를 모두 커버하면 된다.

  예를 들어 anonymous function을 지원하려면 :fn과 매칭되는 process 함수를 만들어야 한다.
  """
